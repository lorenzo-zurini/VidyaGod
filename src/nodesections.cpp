    ...
    const std::string NodePtr = "/NODES/" + std::to_string(N);
            ...
            for (int j = 0; j < (int)Layers.size(); j++)
            {
                ...
                if (LayerType == "VFSZipLayer" || LayerType == "VFSDirLayer" || LayerType == "VFSFileLayer")
                    BuildVfsLayerRows(LBox, LG, n, j, LayerType, LPath);
                else if (LayerType == "RegEdit")
                    BuildRegEditRows(LBox, LG, n, j, LPath);
                    ...
                    BuildDeclareRows(LBox, LG, n, j, LayerType, LPath);
                else if (LayerType == "CustomVar")
                    BuildCustomVarRows(LBox, LG, n, j, LPath);
                Form->addRow(LBox);
            }}


// ---- per-LayerType row builders (P6: the former ~450-line ctor if/else ladder, one method per type) ----
// Each receives the layer's group box + grid, the node/layer indices and the JSON-pointer prefix, and
// builds that type's editing controls. `Layers` is re-derived from the model doc (same object the ctor saw).

void NodeLayersSection::BuildVfsLayerRows(QGroupBox * LBox, QGridLayout * LG, int n, int j, const QString & LayerType, const QString & LPath)
{
    auto &Layers = Model->doc()["NODES"][n]["LAYERS"];
    (void)LBox; (void)Layers; (void)LayerType; (void)LPath;
...
void NodeLayersSection::BuildRegEditRows(QGroupBox * LBox, QGridLayout * LG, int n, int j, const QString & LPath)
{
    auto &Layers = Model->doc()["NODES"][n]["LAYERS"];
    (void)LBox; (void)Layers; (void)LPath;
...
void NodeLayersSection::BuildDllOverrideRows(QGroupBox * LBox, QGridLayout * LG, int n, int j, const QString & LPath)
{
    auto &Layers = Model->doc()["NODES"][n]["LAYERS"];
    (void)LBox; (void)Layers; (void)n; (void)j;
...
void NodeLayersSection::BuildFileEditRows(QGroupBox * LBox, QGridLayout * LG, int n, int j, const QString & LPath)
{
    auto &Layers = Model->doc()["NODES"][n]["LAYERS"];
    (void)LBox; (void)Layers; (void)n; (void)j;
...
void NodeLayersSection::BuildDeclareRows(QGroupBox * LBox, QGridLayout * LG, int n, int j, const QString & LayerType, const QString & LPath)
{
    auto &Layers = Model->doc()["NODES"][n]["LAYERS"];
    (void)LBox; (void)Layers; (void)LayerType;
...
void NodeLayersSection::BuildCustomVarRows(QGroupBox * LBox, QGridLayout * LG, int n, int j, const QString & LPath)
{
    auto &Layers = Model->doc()["NODES"][n]["LAYERS"];
    (void)LBox; (void)Layers; (void)n; (void)j;
int cvrow = 1;
