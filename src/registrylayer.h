...
//Seeds previously-persisted whole-hive files (UserDataPath/REGISTRY/*.reg) into WriteLayerPath before MountVFS so
//they shadow DEFPREFIX. No-op when no persisted regs exist.
bool SeedPersistRegistry(struct ContainerParams &ContainerParams);

//Copies each persisted hive (RuntimePath/<prefixroot>/<hive>.reg) into UserDataPath/REGISTRY/ on Cleanup, capturing
...
//Extracts each persisted registry-subtree from the mounted RuntimePath hives and merges it into the durable store
//UserDataPath/REGKEYS/*.reg on Cleanup. Must run BEFORE unmount. No-op when no regkey persists are declared.
