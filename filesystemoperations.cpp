...
bool FSOps::CreateDirectories (QMap<QString, QString> PathsMap)
{
    for (auto [Name, Path] : PathsMap.asKeyValueRange())
    {
        ...
        if (Dir.mkpath(Path))
        {
            qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[OUT] Created direcotry " << Name << "PATH: " << Path;
        }
        else
        {
            qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[ERR] Could not create directory " << Name << "PATH: " << Path;
            return false;
        }
    }
    return true;
...
bool FSOps::MountZipFileLayer(nlohmann::ordered_json SubComponentJSON, int LayerNumber, QString TempPath, QString PackageFilesPath, QString * UnionFSString, QString PackageName)
{
    ...
    qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[OUT] Mounting ZipFileLayer" << FilePath << "at" << MountDir.path();
    ...
    {
        qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[ERR] Failed to mount zip file layer" << FilePath;
        delete MountZip;
        return false;
    }
}
...
bool FSOps::UnmountZipFileLayer(nlohmann::ordered_json SubComponentJSON, int LayerNumber, QString TempPath, QString ParentPackage)
{
    ...
    qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[OUT] UNMOUNT EXIT STATUS: " << UnmountZips->exitCode();
    delete UnmountZips;
    return true;
}
...
bool FSOps::BuildUnionFS(QString UnionFSString, QString RuntimePath, QString UserDataPath)
{
    ...
    qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[OUT] Building UnionFS RUNTIME.";
    qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[OUT] UnionFSString:" << UnionFSString;
    ...
    {
        qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[OUT] Successfully mounted UnionFS RUNTIME!";
        delete BuildUnionFS;
        return true;
    }
    else
    {
        qDebug().noquote() << QTime::currentTime().toString() << "FSOperations:" << "[ERR] Failed to mount UnionFS RUNTIME!";
        delete BuildUnionFS;
        return false;
    }
}
